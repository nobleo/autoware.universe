...
- `autoware_control_msgs/msg/Control`: message containing both lateral and longitudinal commands.
...
- `timeout_thr_sec`: duration in second after which input messages are discarded.
  - Each time the node receives lateral and longitudinal commands from each controller, it publishes a `Control` message if the following two conditions are met.
